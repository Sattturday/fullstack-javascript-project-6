...
describe('test tasks', () => {
  ...
  describe('CRUD', () => {
    ...
    it('delete by non-creator fails', async () => {
      ...
      const otherUser = await models.user.query()
        .whereNot('id', currentUser.id).first();
  ...
  describe('filters', () => {
    ...
    beforeEach(async () => {
      ...
      existingStatus = await models.taskStatus.query()
        .findOne({ name: testData.statuses.existing.name });
      secondStatus = await models.taskStatus.query()
        .whereNot('id', existingStatus.id).first();
      bugLabel = await models.label.query().findOne({ name: testData.labels.existing.name });
      featureLabel = await models.label.query()
        .whereNot('id', bugLabel.id).first();

      const existingTask = await models.task.query()
